...
export default function CourseModal({isOpen,onClose,id,deleteButtonHandler,addLectureHandler,courseTitle,lectures=[1,2,3,4,5,6,7]}) {
    // const courseTitle = "React Course"


    function VideoCard ({title,description,num,lectureId,courseId,deleteButtonHandler}){
        return <Stack direction={['column','row']} my='8' borderRadius={'lg'} boxShadow={'0 0 10px rgba(107,70,193,0.5)'} justifyContent={['flex','space-between']} p={['4','8']}>
            <Box>
            ...
            <Grid templateColumns={['1fr','3fr 1fr']}>
                <Box px={['0','16']}>
                   ...
                   {lectures.map((item,i)=>(
                    ...
                    key={i}
                        title="React Intro"
                        description='This is a intro video of react'
                        num={i+1}
                        lectureId='adsfcgvbnm'
                        courseId={id}
                ...
                <Box>
                   <form onSubmit={e=>addLectureHandler(e,id,title,description,video)}>
                    <VStack spacing={'4'}>
                        ...
                        <Input focusBorderColor='purple.500' placeholder='Title' value={title} onChange={(e)=>setTitle(e.target.value )} type='text' />

                        <Input focusBorderColor='purple.500' placeholder='Description' value={description} onChange={(e)=>setDescription(e.target.value )} type='text' />
