...
<Input value={createdBy} onChange={e=>setCreatedBy(e.target.value)} placeholder='CreatedBy' type={'text'} focusBorderColor='purple.300' />
<Select value={createdBy} onChange={e=>setCategory(e.target.value)}  focusBorderColor='purple.300'>
  <option value="">Category</option>
  {categories.map(item=>(
    <option key={item} value={item}>{item}</option>
  ))}
</Select>
<Input accept='image/*' required type={'file'} focusBorderColor='purple.500' onChange={changeImageHandler} />
{imagePrev && (
